...
//    console.log(nest);
  ...
  CodeMirror.commands.prologStartElse = function(cm) {
    ...
    if ( state.start == 0 && start.ch == state.end &&
	 !/\S/.test(state.string) )
    { cm.replaceSelection(";   ", "end");
      return;
    }
  ...
  return {
	...
	parserConfig.editor.outOfSync = 0;
	state.curTerm = 0;
	state.curToken = 0;
      }
	...
	style = CodeMirror.enrichStyle(parserConfig, stream, state, type, content, style);
